...
- (UILabel *)placeholderLabel {
    if (!_placeholderLabel) {
        ...
        _placeholderLabel.font = BTDKFont(14);
        _placeholderLabel.textColor = BTDKHexColor(@"#999999");
...
- (void)viewDidLoad {
    ...
    [self configNavigationBarImage:[UIImage imageWithColor:BTDKHexColor(@"#FFFFFF")]];
    [self configNavigationBarShadow:[UIImage imageWithColor:BTDKHexColor(@"#F3F3F3") size:CGSizeMake(ScreenWidth, 1/[UIScreen mainScreen].scale)]];
}
...
- (void)configNavigationBarTitleWithColor:(UIColor *)color {
    [self.navigationController.navigationBar setTitleTextAttributes:@{NSForegroundColorAttributeName:color, NSFontAttributeName:BTDKFont(18)}];
}
...
- (void)showPlaceholderWithImage:(NSString *)image text:(NSString *)text {
    ...
    self.placeholderImage.image = BTDKImage(image);
